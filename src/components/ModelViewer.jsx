  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div className="text-white bg-black/50 px-6 py-2 rounded-full backdrop-blur-md text-[10px] font-black uppercase tracking-[0.4em]">
           {Math.round(progress)} %
        </div>
      )}
    </Html>
...
const ModelInner = ({
  ...
  const isImage = useMemo(() => ['jpg', 'jpeg', 'png', 'webp', 'avif'].includes(ext), [ext]);

  const texture = isImage ? useTexture(url) : null;
  ...
  useFrame((_, dt) => {
    ...
    if (Math.abs(cPar.current.x - tPar.current.x) > 1e-4 || Math.abs(cHov.current.x - tHov.current.x) > 1e-4) need = true;
    if (need) invalidate();
  });

  // Highly aggressive shader to discard checkerboard pattern and backgrounds
  const handleOnBeforeCompile = (shader) => {
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <map_fragment>',
      `
      #include <map_fragment>
      // Aggressive background cut-out
      // Checkerboard usually consists of #ffffff and #cccccc or similar
      float luma = (sampledDiffuseColor.r + sampledDiffuseColor.g + sampledDiffuseColor.b) / 3.0;
      float chroma = max(max(sampledDiffuseColor.r, sampledDiffuseColor.g), sampledDiffuseColor.b) - min(min(sampledDiffuseColor.r, sampledDiffuseColor.g), sampledDiffuseColor.b);
      
      // If it's very bright (white) or very desaturated/gray (checkerboard squares), discard.
      if (luma > 0.82 && chroma < 0.15) discard;
  ...
  return (
    <group ref={outer}>
      <group ref={inner}>
        {isImage ? (
          <mesh>
            <planeGeometry args={[2.8, 1.8, 64, 64]} />
            <meshStandardMaterial 
              ...
              displacementScale={0.3} // Stronger depth for "model effect"
              roughness={0.4}
              metalness={0.1}
            />
          </mesh>
  ...
  return (
    <div style={{ width, height, touchAction: 'pan-y pinch-zoom', position: 'relative' }}>
      ...
      >
        ...
        <ContactShadows position={[0, -0.6, 0]} opacity={0.3} scale={10} blur={2.5} far={1.5} />
